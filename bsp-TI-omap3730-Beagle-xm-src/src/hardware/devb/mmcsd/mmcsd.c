...
static int mmc_ident_mmc_card_powerup(SIM_HBA *hba, int intr)
{
	...
	if(ext->istage > 1){
		...
		if(ext->hccap & (MMC_HCCAP_30V | MMC_HCCAP_33V))
			ocr |= 0x0FFF8000; // high voltage support
...
__SRCVERSION( "$URL: http://svn/product/tags/public/bsp/ntotrunk/ti-omap3730-beagle/latest/src/hardware/devb/mmcsd/mmcsd.c $ $Rev: 400574 $" );
