...
static void
clear_device(const uintptr_t *port, int no_clear_rx_tx_int) 
{
	write_omap(port[OMAP_UART_IER], 0);					// Disable all interrupts
	read_omap(port[OMAP_UART_LSR]);						// Clear Line Status Interrupt
	
	/* Do not clear RX and TX interrupts if requested */
	if(!no_clear_rx_tx_int){
...
ser_attach_intr(DEV_OMAP *dev) {
	...
	clear_device(port, dev->no_clear_rx_tx_int);
...
create_device(TTYINIT_OMAP *dip, unsigned unit, unsigned maxim_xcvr_kick) {
	...
	dev->no_clear_rx_tx_int = dip->no_clear_rx_tx_int; // set no clear flag according to command line option (0 by default)
...
__SRCVERSION( "$URL: http://svn/product/tags/public/bsp/nto641/ti-omap3530-beagle/latest/src/hardware/devc/seromap/init.c $ $Rev: 308279 $" );
