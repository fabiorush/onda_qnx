...
int omap3530_cfg(void *hdl, spi_cfg_t *cfg)
{
	...
	if (!(cfg->mode & SPI_MODE_CSPOL_HIGH))
		ctrl |= OMAP3530_MCSPI_CSPOL_ACTIVE_LOW;	/* Bit Order */ 
	
    if (!(cfg->mode & SPI_MODE_CKPOL_HIGH))
		ctrl |= OMAP3530_MCSPI_POLARIY;	/* Active low polarity */
		
	if (cfg->mode & SPI_MODE_CKPHASE_HALF)
		ctrl |= OMAP3530_MCSPI_PHASE;	/* CPHA 1 Phase */	

#ifdef CPU_OMAP4430
	if (cfg->mode & SPI_MODE_CSHOLD_HIGH)
		ctrl |= OMAP3530_MCSPI_FORCE_MODE_ONE;
    else
	    ctrl &= ~OMAP3530_MCSPI_FORCE_MODE_ONE;
#endif
	...
	*/
    ctrl &= OMAP3530_MCSPI_IS_SOMI_INPUT;
    ctrl &= OMAP3530_MCSPI_DPE1_SIMO_OUTPUT;
    ctrl |= OMAP3530_MCSPI_DPE0_TX_DISABLE;

   if (dev->cs_delay)
   {	
      // Set the CS to start of data delay to 1 1/2 clocks
      ctrl &= OMAP3530_MCSPI_CS_DELAY_MASK;
      ctrl |= OMAP3530_MCSPI_CS_DELAY_ONE_AND_A_HALF;
	}
...
__SRCVERSION( "$URL: http://svn/product/tags/public/bsp/nto641/ti-omap3530-beagle/latest/src/hardware/spi/omap3530/config.c $ $Rev: 308279 $" );
