...
asinfo_string_name(struct asinfo_entry *curr) {
	...
	for( ;; ) {
		...
		new->as = curr;
		chain = new;
		if(curr->owner == AS_NULL_OFF) break;
		curr = (struct asinfo_entry *)((uint8_t *)base + curr->owner); 
	}
	while(chain != NULL) {
...
print_hwinfo(void) {
	...
	while(tag->prefix.size != 0) {
		...
		kprintf("  %d) size:%d tag:%d(%s)", 
				hwi_tag2off(tag), tag->prefix.size, tag->prefix.name, name);
		if(*name >= 'A' && *name <= 'Z') {
...
print_cpuinfo(void) {
	...
	for( i = 0; i < PSP_SYSPAGE->num_cpu; ++i ) {
...
print_syspage_sections(void) {
	...
	kprintf("Header size=0x%x, Total Size=0x%x, #Cpu=%d, Type=%d\n",
		PSP_SYSPAGE->size, PSP_SYSPAGE->total_size,
		PSP_SYSPAGE->num_cpu, PSP_SYSPAGE->type);
