...
/*
 * This should be moved to card detect thread
 */
int sdio_attach_device(void *hdl, uint16_t vid, uint16_t did)
{
	...
	// TODO!!!
	// adjust power
	if (sdio->detect(sdio->hchdl) != MMC_SUCCESS)
		return SDIO_FAILURE;
...
__SRCVERSION( "$URL: http://community.qnx.com/svn/repos/internal_core_networking/branches/6.4.1/trunk/sys/dev_nda/mv8688uap/lib/sdio/sdio_attach_device.c $ $Rev: 252 $" );
